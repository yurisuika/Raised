...
- Replaced direction settings with singular anchor setting for layers.
- Removed displacement settings from layers, now controlled by group offset.
- Removed sync setting from layers.
- Added registration methods with anchor parameter to API so mods may set a default anchor.
...
public class RaisedScreen extends Screen {
    ...
    public static Identifier current = LayerRegistry.HOTBAR;
    ...
    public void setMod(String mod) {
        RaisedScreen.current = LayerRegistry.LAYERS.keySet().stream().sorted(Comparator.comparing(Identifier::toString)).filter(name -> name.getNamespace().equals(mod)).toList().getFirst();
    ...
    public void extractRenderState(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        guiGraphics.centeredText(font, title, PADDING + (WIDGET_WIDTH_WIDE / 2), PADDING + 6, -1);

        guiGraphics.textRenderer().acceptScrollingWithDefaultCenter(Component.literal(Parse.parseNamespace(current)), width - (PADDING + WIDGET_WIDTH_WIDE - WIDGET_WIDTH_SQUARE), width - (PADDING + WIDGET_WIDTH_SQUARE), PADDING, PADDING + WIDGET_HEIGHT);
    }
    ...
    public class LayerList extends SpacedSelectionList<LayerList.Entry> {
        ...
        public void setLayers() {
            clearEntries();
            LayerRegistry.LAYERS.keySet().stream().sorted(Comparator.comparing(Identifier::toString)).filter(identifier -> identifier.getNamespace().equals(RaisedScreen.current.getNamespace())).forEach(name -> addEntry(new Entry(name)));
        }
        ...
        public class Entry extends SpacedSelectionList.Entry<Entry> {
            ...
            @Override
            public void extractContent(GuiGraphicsExtractor guiGraphicsExtractor, int mouseX, int mouseY, boolean bl, float partialTick) {
                guiGraphicsExtractor.textRenderer().acceptScrollingWithDefaultCenter(Component.literal(Parse.parsePath(name)), getX() + WIDGET_WIDTH_SQUARE, getX() + (WIDGET_WIDTH_WIDE - WIDGET_WIDTH_SQUARE), getY(), getY() + WIDGET_HEIGHT);

                guiGraphicsExtractor.blit(RenderPipelines.GUI_TEXTURED, Icon.getLayerIcon(name), getX(), getY(), 0, 0, WIDGET_WIDTH_SQUARE, WIDGET_HEIGHT, WIDGET_WIDTH_SQUARE, WIDGET_HEIGHT);

                guiGraphicsExtractor.blit(RenderPipelines.GUI_TEXTURED, Identifier.fromNamespaceAndPath(Raised.MOD_ID, "textures/gui/direction/" + Configure.getDirectionX(name.toString()).toString().toLowerCase() + "_" + Configure.getDirectionY(name.toString()).toString().toLowerCase() + ".png"), getX() + (WIDGET_WIDTH_WIDE - WIDGET_WIDTH_SQUARE), getY(), 0, 0, WIDGET_WIDTH_SQUARE, WIDGET_HEIGHT, WIDGET_WIDTH_SQUARE, WIDGET_HEIGHT);
            }

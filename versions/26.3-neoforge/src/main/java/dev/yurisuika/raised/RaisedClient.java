...
public class RaisedClient {
    ...
    @EventBusSubscriber(modid = Raised.MOD_ID, value = Dist.CLIENT)
    public static class GameEvents {

        @SubscribeEvent
        public static void registerInputEvents(InputEvent.Key event) {
            while (RaisedOptions.OPTIONS.consumeClick()) {
                Minecraft.getInstance().gui.setScreen(new SelectScreen(null));
            }
        }
    ...
    public static class ModEvents {
        ...
        public static void registerGuiEvents(FMLClientSetupEvent event) {
            NeoForge.EVENT_BUS.register(new Layers.Curated());
        }
        ...
        public static void registerConfigScreens(FMLConstructModEvent event) {
            ModLoadingContext.get().registerExtensionPoint(IConfigScreenFactory.class, () -> (modContainer, screen) -> new SelectScreen(screen));
        }

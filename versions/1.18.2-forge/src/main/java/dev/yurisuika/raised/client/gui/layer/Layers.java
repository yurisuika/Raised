...
public class Layers {
    ...
    public static class Curated {
        ...
        public static final HashMap<IIngameOverlay, ResourceLocation> CURATED_LAYERS = new HashMap<>();
        ...
        public void startTranslate(RenderGameOverlayEvent.PreLayer event) {
            if (Curated.CURATED_LAYERS.containsKey(event.getOverlay())) {
                if (!translated) {
                    translated = true;
                    Translate.start(event.getMatrixStack(), Curated.CURATED_LAYERS.get(event.getOverlay()));
                }
            }
        }
        ...
        public void endTranslate(RenderGameOverlayEvent.PreLayer event) {
            if (Curated.CURATED_LAYERS.containsKey(event.getOverlay()) && event.isCanceled()) {
                if (translated) {
                    translated = false;
                    Translate.end(event.getMatrixStack(), Curated.CURATED_LAYERS.get(event.getOverlay()));
                }
            }
        }

        ...
        dispatcher.register(ClientCommandManager.literal("raised")
                ...
                .then(ClientCommandManager.literal("resources")
                        .then(ClientCommandManager.literal("texture")
                                .executes(commandContext -> {
                                    commandContext.getSource().sendFeedback(Component.translatable("commands.raised.resources.texture.query", Component.translatable(Option.getTexture().getSerializedName())));
                                    return 1;
                                })
                                .then(ClientCommandManager.argument("texture", TextureArgument.texture())
                                        .executes(commandContext -> {
                                            ...
                                            commandContext.getSource().sendFeedback(Component.translatable("commands.raised.resources.texture.set", Component.translatable(Option.getTexture().getSerializedName())));
                                            return 1;
                                        })
                                )
                        )

...
public class LayerList extends ObjectSelectionList<LayerList.Entry> {
    ...
    public int getScrollbarPosition() {
        return getRowLeft() + getRowWidth() + screen.PADDING - 6;
    }
    ...
    public class Entry extends ObjectSelectionList.Entry<Entry> {
        ...
        public void render(PoseStack poseStack, int index, int top, int left, int width, int height, int mouseX, int mouseY, boolean hovering, float partialTick) {
            ((GuiComponentInterface) screen).renderScrollingString(poseStack, Minecraft.getInstance().font, new TextComponent(Parse.parsePath(name)), left + screen.WIDGET_WIDTH_SQUARE, top, left + (screen.WIDGET_WIDTH_WIDE - screen.WIDGET_WIDTH_SQUARE), top + screen.WIDGET_HEIGHT, -1);
            ...
            Minecraft.getInstance().getTextureManager().bind(texture.get());
            ...
            Minecraft.getInstance().getTextureManager().bind(ResourceLocation.tryParse("raised:textures/gui/direction/" + Configure.getDirectionX(name.toString()).toString().toLowerCase() + "_" + Configure.getDirectionY(name.toString()).toString().toLowerCase() + ".png"));

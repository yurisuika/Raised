...
public class LayerList extends ObjectSelectionList<LayerList.Entry> {
    ...
    public class Entry extends ObjectSelectionList.Entry<Entry> {
        ...
        public void render(PoseStack poseStack, int index, int top, int left, int width, int height, int mouseX, int mouseY, boolean hovering, float partialTick) {
            AbstractWidgetInvoker.invokeRenderScrollingString(poseStack, Minecraft.getInstance().font, Component.literal(Parse.parsePath(name)), left + screen.WIDGET_WIDTH_SQUARE, top, left + (screen.WIDGET_WIDTH_WIDE - screen.WIDGET_WIDTH_SQUARE), top + screen.WIDGET_HEIGHT, -1);

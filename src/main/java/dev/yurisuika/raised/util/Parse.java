...
public class Parse {
    ...
    public static List<ResourceLocation> listLoadedIds() {
        ...
        Option.getLayers().forEach((name, layer) -> {
            if (Layers.OTHER_LAYERS.containsKey(ResourceLocation.tryParse(name)) || ResourceLocation.tryParse(name).getNamespace().equals(ResourceLocation.DEFAULT_NAMESPACE)) {
